...
test("service detail pages publish with SEO and seed preserves edited content", async () => {
  ...
  try {
    ...
    for (const service of serviceCatalog) {
      ...
      assert.equal(
        body.blocks.find((b) => b.anchor === "service-examples").items.length,
        3,
      );
      assert.ok(body.blocks.find((b) => b.anchor === "service-approach").image);
      assert.equal(
        body.blocks.find((b) => b.anchor === "service-faq").items.length,
        3,
      );
      assert.ok(pageSchema.safeParse(body).success);
      await request(app)
        .get(path)
        .expect(200)
